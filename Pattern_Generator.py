...
regex_checker = re.compile(r'[가-힣A-Za-z,.?!\'\-\s]+')
...
def Text_Filtering(text):
    ...
    for filter in remove_letters:
        text= text.replace(filter, '')
    for filter, replace_str in replaces:
...
def Basic_Info_Load(
    path: str,
    dataset_label: str,
    language: Optional[Union[Language, Dict[str, Language]]]= None,
    ...
    if type(language) == Language or language is None:
        language_dict = {path: language for path in paths}
...
if __name__ == '__main__':
    ...
    parser.add_argument("-batch", "--batch_size", default= 16, required=False, type= int)
